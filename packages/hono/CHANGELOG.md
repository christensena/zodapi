...
- 4c95e33: `createApp({ zodError })` answers a `ZodError` raised behind a handler as a validation failure instead of letting it fall through to the app's catch-all, normally a bare `500`. Pass `'validationError'` for the same `400` problem document the request validator returns, or a function to turn it into the app's own error type. Off by default. `withZodErrors(option, handler)` is exported for wrapping an error handler directly.
