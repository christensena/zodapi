...
export interface GenerateOptions {
  ...
  /**
   * How OpenAPI documentation (`title`, `description`, `examples`,
   * `deprecated`, operation summaries/tags) is emitted. `'jsdoc'` (the
   * default) as JSDoc comments; `'meta'` as runtime `.meta({...})` calls,
   * `.meta({ id })` component registration, and route documentation fields —
   * full fidelity, spec can be regenerated from the contract; `'none'` drops
   * it entirely.
   */
...
class Generator {
  ...
  private readonly wireHelpers = new Set<string>()
  ...
  private readonly docs: DocsMode
  ...
  constructor(
    ...
    this.docs = options?.docs ?? 'jsdoc'
    this.ctx.docs = this.docs
  }
  ...
  generate(): string {
    ...
    const componentDecls = order.map((name) => {
      ...
      const expr = convertSchema(schema, this.ctx)
      const code = this.docs === 'meta' ? withMeta(expr.code, [`id: ${json(name)}`]) : expr.code
      this.declared.add(name)
      const ident = this.names.lookup(name)
      const jsdoc =
        this.docs === 'jsdoc' && isObject(schema)
          ? schemaJsdoc(schema, '')
              .map((l) => `${l}\n`)
              .join('')
          : ''
      const constDecl = `${jsdoc}export const ${ident} = ${code}\n`
    ...
    const coreImports = [
      'route',
      ...(this.usesQueryArray ? ['queryArray'] : []),
      ...this.wireHelpers,
    ]
    const imports = [
      `import { ${coreImports.sort().join(', ')} } from '@zodapi/core'`,
      `import { z } from 'zod'`,
    ]
    const routesArray =
  ...
  private emitRoute(
    ...
    const lines: string[] = []
    if (this.docs === 'jsdoc') lines.push(...this.routeJsdoc(op))
    lines.push(`export const ${ident} = route({`)
    lines.push(`  method: ${json(method)},`)
    lines.push(`  path: ${json(path)},`)
    if (operationId !== undefined) lines.push(`  alias: ${json(operationId)},`)
    if (this.docs === 'meta') {
      if (operationId !== undefined) lines.push(`  operationId: ${json(operationId)},`)
  ...
  /** JSDoc block for an operation: summary + description body, `@tags` line. */
  private routeJsdoc(op: JsonSchema): string[] {
    const body: string[] = []
    if (typeof op['summary'] === 'string') body.push(op['summary'])
    if (typeof op['description'] === 'string') {
      if (body.length > 0) body.push('')
      body.push(...op['description'].split('\n'))
    }
    const tags =
      Array.isArray(op['tags']) && op['tags'].length > 0 ? [`@tags ${op['tags'].join(', ')}`] : []
    return jsdocLines(body, tags, '')
  }
  ...
  private requestLines(op: JsonSchema, pathItem: JsonSchema): string[] {
    ...
    for (const group of groups) {
      ...
      const entries = members.flatMap((p) => {
        ...
        return [...this.parameterJsdoc(p), `      ${name}: ${code},`]
      })
    ...
    if (p.in === 'query' && isObject(schema) && schema['type'] === 'array') {
      ...
      const items =
        schema['items'] === undefined
          ? 'z.unknown()'
          : this.wireWrap(this.expr(schema['items'], '      '), schema['items'], p.in)
      const description =
        this.docs === 'meta' && typeof p.description === 'string'
          ? `.meta({ description: ${json(p.description)} })`
          : ''
      return `queryArray(${items})${description}`
    }
    let code = this.wireWrap(this.expr(schema, '      '), schema, p.in)
    if (
      this.docs === 'meta' &&
      typeof p.description === 'string' &&
      !(isObject(schema) && 'description' in schema)
    ) {
  ...
  /**
   * Path and query values arrive as raw strings, so a numeric or boolean schema
   * would reject every request. `wireNumber`/`wireBoolean` preprocess the string
   * without altering the schema, keeping the generated document identical to the
   * source one — `z.coerce.number()`/`z.stringbool()` would not.
   */
  private wireWrap(code: string, schema: unknown, location: ParameterObject['in']): string {
    if (location !== 'path' && location !== 'query') return code
    const helper = this.wireHelperFor(schema)
    if (helper === undefined) return code
    this.wireHelpers.add(helper)
    return `${helper}(${code})`
  }

  /** `wireNumber`/`wireBoolean` for a numeric or boolean parameter schema, else `undefined`. */
  private wireHelperFor(schema: unknown, seenRefs = new Set<string>()): string | undefined {
    if (!isObject(schema)) return undefined
    const ref = schema['$ref']
    if (typeof ref === 'string') {
      if (seenRefs.has(ref)) return undefined
      seenRefs.add(ref)
      return this.wireHelperFor(this.ctx.resolveComponentSchema(ref), seenRefs)
    }
    const type = Array.isArray(schema['type'])
      ? schema['type'].filter((t) => t !== 'null')
      : [schema['type']]
    if (type.length !== 1) return undefined
    if (type[0] === 'number' || type[0] === 'integer') return 'wireNumber'
    if (type[0] === 'boolean') return 'wireBoolean'
    return undefined
  }

  /** JSDoc lines for a parameter, from its schema keywords plus the parameter description. */
  private parameterJsdoc(p: ParameterObject): string[] {
    if (this.docs !== 'jsdoc') return []
    const schema = isObject(p.schema) ? p.schema : {}
    const description =
      typeof schema['description'] === 'string' ? schema['description'] : p.description
    return schemaJsdoc({ ...schema, ...(description !== undefined && { description }) }, '      ')
  }
  ...
  private contentLines(content: JsonSchema, indent: string): string[] {
    ...
    for (const [media, mediaDef] of Object.entries(content)) {
      ...
      if (mediaDef['schema'] === undefined) {
        ...
        const jsdoc =
          this.docs === 'jsdoc' && isObject(mediaDef['schema'])
            ? schemaJsdoc(mediaDef['schema'], `${indent}    `)
            : []
        lines.push(
          `${indent}  ${json(media)}: {`,
          ...jsdoc,
