...
class Generator {
  ...
  generate(): string {
    ...
    // RouteDef plus the documentation fields OpenAPI operations carry (meta mode only).
    const routeType =
      this.docs === 'meta'
    ...
    return [
      ...
      imports.join('\n'),
      ...(routeDecls.length > 0 && routeType !== undefined ? [routeType] : []),
      ...dateCodecDecls,
      ...componentDecls,
      ...routeDecls,
      routesArray,
  ...
  private emitRoute(
    ...
    lines.push('  },')
    lines.push(`} as const satisfies ${this.docs === 'meta' ? 'GeneratedRoute' : 'RouteDef'}`)
    out.push(`${lines.join('\n')}\n`)
    return ident
  }
