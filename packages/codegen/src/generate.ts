...
import { convertSchema, type ConvertContext, type JsonSchema } from './schema-to-zod.js'
 ...
 */
export function generateContract(doc: unknown): string {
  if (!isObject(doc)) throw new Error('expected a parsed OpenAPI document object')
  return new Generator(doc).generate()
}
